...
public class ProceduralTerrainVolume : MonoBehaviour
{
	...
	void Start()
	{
		...
		volumeRenderer.material.SetTextureScale("_Tex1", new Vector2(0.03f, 0.03f));	
		volumeRenderer.material.SetTextureScale("_Tex2", new Vector2(0.03f, 0.03f));
        ...
        //float rockScale = 36.0f;
        float rockScale = Random.Range(36f,42f);		
        float invRockScale = 1f / rockScale;
		...
		for(int z = 0; z < depth; z++)
		{
			for(int y = height-1; y > 0; y--)
			{
				for(int x = 0; x < width; x++)
				{
					...
					simplexNoiseValue -= altitude*1.25f;
